    ...
    function OracleServer(n) {
        ...
        node.getPool = function (config, cb) {
            if (!node.pool) {
                ...
                oracledb.createPool(config, function (err, pl) {
                    if (err) {
                        return cb("".concat(err, " config=").concat(JSON.stringify(config)));
                    }
        ...
        node.claimPoolConnection = function (requestingNode, msg) {
            ...
            if (!node.connection && !node.connectionInProgress) {
                ...
                node.getPool(config, function (err, pool) {
                    if (err) {
                        node.status.emit("error", err);
                        requestingNode.error("Oracle-server error connection to " + node.connectString + ": " + err.message, msg);
                        if (err.message.match(/1017/g)) {
            ...
            if (node.connection) {
                ...
                node.connection.execute(query, values, options, function (err, result) {
                    if (err) {
                        ...
                        // Hack: Single out bound cursor that MUST BE mapped in query as
                        // :cursor and values as cursor: { type: oracledb.CURSOR, dir: oracledb.BIND_OUT }!
                        if (result.outBinds && result.outBinds.cursor) {
                            // Future: for each outBinds that is a cursor,
                            //   fetchRowsFromResultSet(requestingNode, cursor.resultSet,resultSetLimit,sendMessageFn)
                            //   multiple output nodes! requestingNode.send([msg,null]);
                            node.fetchRowsFromResultSet(requestingNode, result.outBinds.cursor, resultSetLimit, msg);
                            requestingNode.log("Oracle query with cursor result rows sent");
                        }
                        else {
                            switch (resultAction) {
                                case "single":
